...
const fetchDashboardData = (
  ...
  const query = `select ${dateFieldName} as today, name , (popularity * 10000) as popularity,(1-((anti_count * 1.5) / m_count)) as anti_ratio,word1, word2, word3,${rankFieldName}, (select count(name) from ${tableName} where ${dateFieldName} like today) as total_community from ${tableName} where name like ? and (${dateFieldName} between ? and ? ) order by today asc`;
